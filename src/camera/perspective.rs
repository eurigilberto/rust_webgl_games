...
impl PerspectiveCamera {
    ...
    const INIT_UP: Vec3 = glam::Vec3::Y;
    ...
    pub fn update_perspective_props(
        ...
        if let Some(fov) = fov {
            self.perspective_props.fov = fov;
        }
    }

    pub fn generate_camera_matrices(&mut self, camera_offset: Vec3) -> CameraMatrices {
        let transform_matrix = glam::Mat4::look_at_rh(camera_offset, camera_offset + self.view_forward, self.view_up);
        let view_matrix = transform_matrix.inverse();
        let projection_matrix =glam::Mat4::perspective_rh_gl(
            self.perspective_props.fov,
